 ...
 */
Ftp.prototype.getHash = function (remoteDir, cb) {
  this.get( remoteDir + '.gulpftp', function (err, stream) {
    if( err ) {
      if( err.code == 550 ) {
        return cb(null, '');
      }
...
Ftp.prototype.place = function (files, each, remoteDir, finishCb) {
  var self = this;

  async.eachLimit(Object.keys(files), 10, function (filename, next) {

    // console.log(remoteDir)
    var filenamePath = remoteDir + filename;

    if( files[filename] ) {
      self.upload(filename, filenamePath, function (err) {
        each(err, { name: filenamePath, action: 'uploaded' }, next);
      });
    } else {
      self.remove(filenamePath, function (err) {
        each(err, { name: filenamePath, action: 'deleted' }, next);
      });
    }
  }, finishCb);
}
